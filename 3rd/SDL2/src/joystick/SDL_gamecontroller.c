...
static SDL_JoystickGUID s_fastwareUID;
...
struct _SDL_GameController {
  ...
  struct _SDL_GameController
      *next; /* pointer to next game controller we have allocated */
};
...
static void SDL_LoadVIDPIDListFromHint(const char *hint,
  ...
  while ((spot = SDL_strstr(spot, "0x")) != NULL) {
    ...
    if (list->num_entries == list->max_entries) {
      ...
      Uint32 *entries = (Uint32 *)SDL_realloc(
          list->entries, max_entries * sizeof(*list->entries));
      if (entries == NULL) {
...
static void SDLCALL SDL_GameControllerIgnoreDevicesChanged(void *userdata,
                                                           const char *name,
                                                           const char *oldValue,
                                                           const char *hint) {
...
static void SDLCALL SDL_GameControllerIgnoreDevicesExceptChanged(
    void *userdata, const char *name, const char *oldValue, const char *hint) {
  SDL_LoadVIDPIDListFromHint(hint, &SDL_allowed_controllers);
}

static ControllerMapping_t *
SDL_PrivateAddMappingForGUID(SDL_JoystickGUID jGUID, const char *mappingString,
                             SDL_bool *existing,
                             SDL_ControllerMappingPriority priority);
...
static void UpdateEventsForDeviceRemoval(int device_index) {
  ...
  num_events =
      SDL_PeepEvents(events, num_events, SDL_GETEVENT,
  ...
  for (i = 0; i < num_events; ++i) {
    if (events[i].cdevice.which < device_index) {
      ...
      SDL_memmove(&events[i], &events[i + 1],
                  sizeof(*events) * (num_events - (i + 1)));
      --i;
      --num_events;
    } else {
...
static void ResetOutput(SDL_GameController *gamecontroller,
  ...
  if (bind->outputType == SDL_CONTROLLER_BINDTYPE_AXIS) {
    ...
    SDL_PrivateGameControllerButton(gamecontroller, bind->output.button,
                                    SDL_RELEASED);
  }
}

static void HandleJoystickAxis(SDL_GameController *gamecontroller, int axis,
                               int value) {
  int i;
  SDL_ExtendedGameControllerBind *last_match =
      gamecontroller->last_match_axis[axis];
    ...
    if (match->outputType == SDL_CONTROLLER_BINDTYPE_AXIS) {
      if (match->input.axis.axis_min != match->output.axis.axis_min ||
        ...
        float normalized_value =
            (float)(value - match->input.axis.axis_min) /
            (match->input.axis.axis_max - match->input.axis.axis_min);
        value = match->output.axis.axis_min +
                (int)(normalized_value * (match->output.axis.axis_max -
                                          match->output.axis.axis_min));
      }
      SDL_PrivateGameControllerAxis(gamecontroller, match->output.axis.axis,
                                    (Sint16)value);
    } else {
      Uint8 state;
      int threshold =
      ...
      SDL_PrivateGameControllerButton(gamecontroller, match->output.button,
                                      state);
    }
  }
...
static void HandleJoystickButton(SDL_GameController *gamecontroller, int button,
                                 Uint8 state) {
  int i;
  ...
  for (i = 0; i < gamecontroller->num_bindings; ++i) {
    ...
    if (binding->inputType == SDL_CONTROLLER_BINDTYPE_BUTTON &&
      ...
      if (binding->outputType == SDL_CONTROLLER_BINDTYPE_AXIS) {
        int value = state ? binding->output.axis.axis_max
...
static void HandleJoystickHat(SDL_GameController *gamecontroller, int hat,
                              Uint8 value) {
  int i;
...
static void RecenterGameController(SDL_GameController *gamecontroller) {
  ...
  for (button = (SDL_GameControllerButton)0; button < SDL_CONTROLLER_BUTTON_MAX;
       button++) {
  ...
  for (axis = (SDL_GameControllerAxis)0; axis < SDL_CONTROLLER_AXIS_MAX;
       axis++) {
 ...
 */
static int SDLCALL SDL_GameControllerEventWatcher(void *userdata,
                                                  SDL_Event *event) {
  switch (event->type) {
  case SDL_JOYAXISMOTION: {
    ...
    while (controllerlist) {
      if (controllerlist->joystick->instance_id == event->jaxis.which) {
        HandleJoystickAxis(controllerlist, event->jaxis.axis,
                           event->jaxis.value);
        break;
      }
...
#ifdef __ANDROID__
/*
 * Helper function to guess at a mapping based on the elements reported for this
 * controller
 */
static ControllerMapping_t *
SDL_CreateMappingForAndroidController(SDL_JoystickGUID guid) {
  const int face_button_mask =
      ((1 << SDL_CONTROLLER_BUTTON_A) | (1 << SDL_CONTROLLER_BUTTON_B) |
       (1 << SDL_CONTROLLER_BUTTON_X) | (1 << SDL_CONTROLLER_BUTTON_Y));
  SDL_bool existing;
  ...
  if (button_mask & (1 << SDL_CONTROLLER_BUTTON_GUIDE)) {
    /* The guide button generally isn't functional (or acts as a home button) on
     * most Android controllers before Android 11 */
  ...
  return SDL_PrivateAddMappingForGUID(guid, mapping_string, &existing,
                                      SDL_CONTROLLER_MAPPING_PRIORITY_DEFAULT);
}
#endif /* __ANDROID__ */
 ...
 */
static ControllerMapping_t *
SDL_CreateMappingForHIDAPIController(SDL_JoystickGUID guid) {
  SDL_bool existing;
  char mapping_string[1024];
  Uint16 vendor;
  Uint16 product;
  ...
  if ((vendor == USB_VENDOR_NINTENDO &&
      ...
      (vendor == USB_VENDOR_SHENZHEN &&
       product == USB_PRODUCT_EVORETRO_GAMECUBE_ADAPTER)) {
    /* GameCube driver has 12 buttons and 6 axes */
    SDL_strlcat(mapping_string,
                "a:b0,b:b1,dpdown:b6,dpleft:b4,dpright:b5,dpup:b7,lefttrigger:"
                "a4,leftx:a0,lefty:a1,rightshoulder:b9,righttrigger:a5,rightx:"
                "a2,righty:a3,start:b8,x:b2,y:b3,",
                sizeof(mapping_string));
  } else {
    /* All other controllers have the standard set of 19 buttons and 6 axes */
    SDL_strlcat(mapping_string,
                "a:b0,b:b1,back:b4,dpdown:b12,dpleft:b13,dpright:b14,dpup:b11,"
                "guide:b5,leftshoulder:b9,leftstick:b7,lefttrigger:a4,leftx:a0,"
                "lefty:a1,rightshoulder:b10,rightstick:b8,righttrigger:a5,"
                "rightx:a2,righty:a3,start:b6,x:b2,y:b3,",
                sizeof(mapping_string));
    ...
    if (SDL_IsJoystickXboxSeriesX(vendor, product)) {
      ...
      SDL_strlcat(mapping_string, "paddle1:b16,paddle2:b15,",
                  sizeof(mapping_string));
    } else {
      ...
      case SDL_CONTROLLER_TYPE_PS5:
        /* PS5 controllers have a microphone button and an additional touchpad
         * button */
        SDL_strlcat(mapping_string, "touchpad:b15,misc1:b16",
                    sizeof(mapping_string));
        break;
      case SDL_CONTROLLER_TYPE_NINTENDO_SWITCH_PRO:
        ...
        if (SDL_IsJoystickNintendoSwitchJoyConLeft(vendor, product)) {
          SDL_strlcat(mapping_string, "paddle2:b17,paddle4:b19,",
                      sizeof(mapping_string));
        } else if (SDL_IsJoystickNintendoSwitchJoyConRight(vendor, product)) {
          SDL_strlcat(mapping_string, "paddle1:b16,paddle3:b18,",
                      sizeof(mapping_string));
        }
        break;
      ...
      case SDL_CONTROLLER_TYPE_GOOGLE_STADIA:
        /* The Google Stadia controller has a share button and a Google
         * Assistant button */
        SDL_strlcat(mapping_string, "misc1:b15,", sizeof(mapping_string));
        break;
      default:
 ...
 */
static ControllerMapping_t *
SDL_CreateMappingForRAWINPUTController(SDL_JoystickGUID guid) {
  SDL_bool existing;
  char mapping_string[1024];

  SDL_strlcpy(mapping_string, "none,*,", sizeof(mapping_string));
  SDL_strlcat(mapping_string,
              "a:b0,b:b1,x:b2,y:b3,back:b6,guide:b10,start:b7,leftstick:b8,"
              "rightstick:b9,leftshoulder:b4,rightshoulder:b5,dpup:h0.1,dpdown:"
              "h0.4,dpleft:h0.8,dpright:h0.2,leftx:a0,lefty:a1,rightx:a2,"
 ...
 */
static ControllerMapping_t *
SDL_CreateMappingForWGIController(SDL_JoystickGUID guid) {
  SDL_bool existing;
  char mapping_string[1024];
  ...
  SDL_strlcat(mapping_string,
              ...
              "leftshoulder:b4,rightshoulder:b5,dpup:b10,dpdown:b12,dpleft:b13,"
              "dpright:b11,leftx:a1,lefty:a0~,rightx:a3,righty:a2~,lefttrigger:"
              "a4,righttrigger:a5,",
              sizeof(mapping_string));
...
/*
 * Helper function to scan the mappings database for a controller with the
 * specified GUID
 */
static ControllerMapping_t *
SDL_PrivateGetControllerMappingForGUID(SDL_JoystickGUID guid,
...
static const char *map_StringForControllerAxis[] = {
    "leftx", "lefty", "rightx", "righty", "lefttrigger", "righttrigger", NULL};
...
static const char *map_StringForControllerButton[] = {"a",
                                                      "b",
                                                      "x",
                                                      "y",
                                                      "back",
...
SDL_GameControllerGetStringForButton(SDL_GameControllerButton axis) {
  if (axis > SDL_CONTROLLER_BUTTON_INVALID &&
      axis < SDL_CONTROLLER_BUTTON_MAX) {
...
/*
 * given a controller button name and a joystick name update our mapping
 * structure with it
 */
static void
SDL_PrivateGameControllerParseElement(SDL_GameController *gamecontroller,
  ...
  if (szJoystickButton[0] == 'a' &&
      SDL_isdigit((unsigned char)szJoystickButton[1])) {
    ...
    bind.input.button = SDL_atoi(&szJoystickButton[1]);
             ...
             SDL_isdigit((unsigned char)szJoystickButton[1]) &&
             szJoystickButton[2] == '.' &&
             SDL_isdigit((unsigned char)szJoystickButton[3])) {
 ...
 */
static void SDL_PrivateGameControllerParseControllerConfigString(
    SDL_GameController *gamecontroller, const char *pchString) {
  ...
  while (pchPos && *pchPos) {
    if (*pchPos == ':') {
      ...
      SDL_PrivateGameControllerParseElement(gamecontroller, szGameButton,
                                            szJoystickButton);
      SDL_zeroa(szGameButton);
      SDL_zeroa(szJoystickButton);
  ...
  /* No more values if the string was terminated by a comma. Don't report an
   * error. */
...
static void SDL_PrivateLoadButtonMapping(SDL_GameController *gamecontroller,
  ...
  if (gamecontroller->joystick->naxes) {
    SDL_memset(gamecontroller->last_match_axis, 0,
               gamecontroller->joystick->naxes *
                   sizeof(*gamecontroller->last_match_axis));
  }

  SDL_PrivateGameControllerParseControllerConfigString(gamecontroller,
                                                       pchMapping);
    ...
    if (binding->inputType == SDL_CONTROLLER_BINDTYPE_AXIS &&
      ...
      if (binding->input.axis.axis < gamecontroller->joystick->naxes) {
        gamecontroller->joystick->axes[binding->input.axis.axis].value =
            gamecontroller->joystick->axes[binding->input.axis.axis].zero =
                (Sint16)binding->input.axis.axis_min;
      }
    }
  }
}
 ...
 */
static char *
SDL_PrivateGetControllerGUIDFromMappingString(const char *pMapping) {
 ...
 */
static char *
SDL_PrivateGetControllerNameFromMappingString(const char *pMapping) {
 ...
 */
static char *
SDL_PrivateGetControllerMappingFromMappingString(const char *pMapping) {
  const char *pFirstComma, *pSecondComma;
  ...
  return SDL_strdup(pSecondComma +
                    1); /* mapping is everything after the 3rd comma */
}

/*
 * Helper function to refresh a mapping
 */
static void SDL_PrivateGameControllerRefreshMapping(
    ControllerMapping_t *pControllerMapping) {
  SDL_GameController *gamecontrollerlist = SDL_gamecontrollers;
  while (gamecontrollerlist) {
    if (!SDL_memcmp(&gamecontrollerlist->joystick->guid,
                    &pControllerMapping->guid,
                    sizeof(pControllerMapping->guid))) {
      /* Not really threadsafe.  Should this lock access within
  ...
  if (pControllerMapping) {
    ...
    *existing = SDL_TRUE;
    ...
    if (s_pSupportedControllers) {
      ...
      for (pPrevMapping = s_pSupportedControllers,
          pCurrMapping = pPrevMapping->next;
           pCurrMapping;
 ...
 */
static ControllerMapping_t *
SDL_PrivateGetControllerMappingForNameAndGUID(const char *name,
                                              SDL_JoystickGUID guid) {
  ...
  if (!mapping && name) {
    if (SDL_strstr(name, "Xbox 360 Wireless Receiver")) {
      ...
      SDL_bool existing;
      mapping = SDL_PrivateAddMappingForGUID(
          guid,
          "none,X360 Wireless "
          "Controller,a:b0,b:b1,back:b6,dpdown:b14,dpleft:b11,dpright:b12,dpup:"
          "b13,guide:b8,leftshoulder:b4,leftstick:b9,lefttrigger:a2,leftx:a0,"
          "lefty:a1,rightshoulder:b5,rightstick:b10,righttrigger:a5,rightx:a3,"
          "righty:a4,start:b7,x:b2,y:b3",
          &existing, SDL_CONTROLLER_MAPPING_PRIORITY_DEFAULT);
    } else if (SDL_strstr(name, "Xbox") || SDL_strstr(name, "X-Box") ||
...
static void SDL_PrivateAppendToMappingString(char *mapping_string,
  ...
  case EMappingKind_Hat:
    SDL_snprintf(buffer, sizeof(buffer), "h%i.%i", mapping->target >> 4,
                 mapping->target & 0x0F);
    break;
  default:
...
static ControllerMapping_t *SDL_PrivateGenerateAutomaticControllerMapping(
    const char *name, SDL_JoystickGUID guid, SDL_GamepadMapping *raw_map) {
  SDL_bool existing;
  ...
  SDL_PrivateAppendToMappingString(mapping, sizeof(mapping), "back",
                                   &raw_map->back);
  SDL_PrivateAppendToMappingString(mapping, sizeof(mapping), "guide",
                                   &raw_map->guide);
  SDL_PrivateAppendToMappingString(mapping, sizeof(mapping), "start",
                                   &raw_map->start);
  SDL_PrivateAppendToMappingString(mapping, sizeof(mapping), "leftstick",
                                   &raw_map->leftstick);
  SDL_PrivateAppendToMappingString(mapping, sizeof(mapping), "rightstick",
                                   &raw_map->rightstick);
  SDL_PrivateAppendToMappingString(mapping, sizeof(mapping), "leftshoulder",
                                   &raw_map->leftshoulder);
  SDL_PrivateAppendToMappingString(mapping, sizeof(mapping), "rightshoulder",
                                   &raw_map->rightshoulder);
  SDL_PrivateAppendToMappingString(mapping, sizeof(mapping), "dpup",
                                   &raw_map->dpup);
  SDL_PrivateAppendToMappingString(mapping, sizeof(mapping), "dpdown",
                                   &raw_map->dpdown);
  SDL_PrivateAppendToMappingString(mapping, sizeof(mapping), "dpleft",
                                   &raw_map->dpleft);
  SDL_PrivateAppendToMappingString(mapping, sizeof(mapping), "dpright",
                                   &raw_map->dpright);
  SDL_PrivateAppendToMappingString(mapping, sizeof(mapping), "leftx",
                                   &raw_map->leftx);
  SDL_PrivateAppendToMappingString(mapping, sizeof(mapping), "lefty",
                                   &raw_map->lefty);
  SDL_PrivateAppendToMappingString(mapping, sizeof(mapping), "rightx",
                                   &raw_map->rightx);
  SDL_PrivateAppendToMappingString(mapping, sizeof(mapping), "righty",
                                   &raw_map->righty);
  SDL_PrivateAppendToMappingString(mapping, sizeof(mapping), "lefttrigger",
                                   &raw_map->lefttrigger);
  SDL_PrivateAppendToMappingString(mapping, sizeof(mapping), "righttrigger",
                                   &raw_map->righttrigger);
  ...
  return SDL_PrivateAddMappingForGUID(guid, mapping, &existing,
                                      SDL_CONTROLLER_MAPPING_PRIORITY_DEFAULT);
}
...
static ControllerMapping_t *SDL_PrivateGetControllerMapping(int device_index) {
  ...
  if (!mapping) {
    ...
    if (SDL_PrivateJoystickGetAutoGamepadMapping(device_index, &raw_map)) {
      mapping =
          SDL_PrivateGenerateAutomaticControllerMapping(name, guid, &raw_map);
    }
  }
...
SDL_PrivateGameControllerAddMapping(const char *mappingString,
  ...
  { /* Extract and verify the SDK version */
    ...
    if (tmp != NULL) {
      ...
      if (!(SDL_GetAndroidSDKVersion() >= SDL_atoi(tmp))) {
        return SDL_SetError("SDK version %d < minimum version %d",
                            SDL_GetAndroidSDKVersion(), SDL_atoi(tmp));
      }
    }
    ...
    if (tmp != NULL) {
      ...
      if (!(SDL_GetAndroidSDKVersion() <= SDL_atoi(tmp))) {
        return SDL_SetError("SDK version %d > maximum version %d",
                            SDL_GetAndroidSDKVersion(), SDL_atoi(tmp));
      }
    }
  }
  ...
  pControllerMapping =
      SDL_PrivateAddMappingForGUID(jGUID, mappingString, &existing, priority);
  if (!pControllerMapping) {
    return -1;
  }
...
int SDL_GameControllerAddMapping(const char *mappingString) {
  return SDL_PrivateGameControllerAddMapping(
      mappingString, SDL_CONTROLLER_MAPPING_PRIORITY_API);
}
...
int SDL_GameControllerNumMappings(void) {
  ...
  for (mapping = s_pSupportedControllers; mapping; mapping = mapping->next) {
    if (SDL_memcmp(&mapping->guid, &s_fastwareUID, sizeof(mapping->guid)) ==
        0) {
      continue;
    }
    ++num_mappings;
 ...
 */
static char *CreateMappingString(ControllerMapping_t *mapping,
                                 SDL_JoystickGUID guid) {
  ...
  if (pPlatformString) {
    pPlatformString =
        SDL_strstr(pPlatformString + 1, SDL_CONTROLLER_PLATFORM_FIELD);
    if (pPlatformString) {
      *pPlatformString = '\0';
    }
  }
...
char *SDL_GameControllerMappingForGUID(SDL_JoystickGUID guid) {
  ControllerMapping_t *mapping =
      SDL_PrivateGetControllerMappingForGUID(guid, SDL_FALSE);
  if (mapping) {
...
#if defined(__ANDROID__)
  return SDL_snprintf(path, size, "%s/controller_map.txt",
                      SDL_AndroidGetInternalStoragePath()) < size;
#else
  return SDL_FALSE;
#endif
}
...
/*
 * Initialize the game controller system, mostly load our DB of controller
 * config mappings
 */
int SDL_GameControllerInitMappings(void) {
  ...
  while (pMappingString) {
    SDL_PrivateGameControllerAddMapping(
        pMappingString, SDL_CONTROLLER_MAPPING_PRIORITY_DEFAULT);

    i++;
    pMappingString = s_ControllerMappings[i];
  }

  if (SDL_GetControllerMappingFilePath(szControllerMapPath,
                                       sizeof(szControllerMapPath))) {
...
const char *SDL_GameControllerNameForIndex(int device_index) {
  ControllerMapping_t *pSupportedController =
      SDL_PrivateGetControllerMapping(device_index);
...
SDL_GameControllerType SDL_GameControllerTypeForIndex(int joystick_index) {
  return SDL_GetJoystickGameControllerTypeFromGUID(
      SDL_JoystickGetDeviceGUID(joystick_index),
...
SDL_bool SDL_IsGameControllerNameAndGUID(const char *name,
                                         SDL_JoystickGUID guid) {
  ControllerMapping_t *pSupportedController =
      SDL_PrivateGetControllerMappingForNameAndGUID(name, guid);
  if (pSupportedController) {
    return SDL_TRUE;
  }
  return SDL_FALSE;
...
SDL_bool SDL_ShouldIgnoreGameController(const char *name,
  ...
  if (SDL_GetHintBoolean("SDL_GAMECONTROLLER_ALLOW_STEAM_VIRTUAL_GAMEPAD",
                         SDL_FALSE)) {
    /* We shouldn't ignore Steam's virtual gamepad since it's using the hints to
     * filter out the real controllers so it can remap input for the virtual
     * controller */
    /* https://partner.steamgames.com/doc/features/steam_controller/steam_input_gamepad_emulation_bestpractices
     */
    SDL_bool bSteamVirtualGamepad = SDL_FALSE;
#if defined(__LINUX__)
    bSteamVirtualGamepad = (vendor == USB_VENDOR_VALVE &&
                            product == USB_PRODUCT_STEAM_VIRTUAL_GAMEPAD);
#elif defined(__MACOSX__)
    bSteamVirtualGamepad =
        (vendor == USB_VENDOR_MICROSOFT &&
         product == USB_PRODUCT_XBOX360_WIRED_CONTROLLER && version == 1);
...
SDL_GameController *SDL_GameControllerOpen(int device_index) {
  ...
  if (gamecontroller->joystick->naxes) {
    gamecontroller->last_match_axis =
        (SDL_ExtendedGameControllerBind **)SDL_calloc(
            gamecontroller->joystick->naxes,
            sizeof(*gamecontroller->last_match_axis));
  ...
  if (gamecontroller->joystick->nhats) {
    gamecontroller->last_hat_mask =
        (Uint8 *)SDL_calloc(gamecontroller->joystick->nhats,
...
Sint16 SDL_GameControllerGetAxis(SDL_GameController *gamecontroller,
  ...
  for (i = 0; i < gamecontroller->num_bindings; ++i) {
    ...
    if (binding->outputType == SDL_CONTROLLER_BINDTYPE_AXIS &&
      ...
      if (binding->inputType == SDL_CONTROLLER_BINDTYPE_AXIS) {
        value = SDL_JoystickGetAxis(gamecontroller->joystick,
        ...
        value = SDL_JoystickGetButton(gamecontroller->joystick,
        ...
        int hat_mask = SDL_JoystickGetHat(gamecontroller->joystick,
      ...
      /* If the value is zero, there might be another binding that makes it
       * non-zero */
  ...
  for (i = 0; i < gamecontroller->num_bindings; ++i) {
    ...
    if (binding->outputType == SDL_CONTROLLER_BINDTYPE_BUTTON &&
      ...
      if (binding->inputType == SDL_CONTROLLER_BINDTYPE_AXIS) {
        ...
        int value = SDL_JoystickGetAxis(gamecontroller->joystick,
                                        binding->input.axis.axis);
        int threshold =
        ...
        return SDL_JoystickGetButton(gamecontroller->joystick,
                                     binding->input.button);
      } else if (binding->inputType == SDL_CONTROLLER_BINDTYPE_HAT) {
        int hat_mask = SDL_JoystickGetHat(gamecontroller->joystick,
                                          binding->input.hat.hat);
        return (hat_mask & binding->input.hat.hat_mask) ? SDL_PRESSED
                                                        : SDL_RELEASED;
      }
    }
  }
...
SDL_GameControllerGetType(SDL_GameController *gamecontroller) {
  return SDL_GetJoystickGameControllerTypeFromGUID(
      SDL_JoystickGetGUID(SDL_GameControllerGetJoystick(gamecontroller)),
      SDL_JoystickName(SDL_GameControllerGetJoystick(gamecontroller)));
}

int SDL_GameControllerGetPlayerIndex(SDL_GameController *gamecontroller) {
  return SDL_JoystickGetPlayerIndex(
...
void SDL_GameControllerSetPlayerIndex(SDL_GameController *gamecontroller,
                                      int player_index) {
  SDL_JoystickSetPlayerIndex(SDL_GameControllerGetJoystick(gamecontroller),
                             player_index);
}
...
SDL_GameControllerGetBindForAxis(SDL_GameController *gamecontroller,
  ...
  for (i = 0; i < gamecontroller->num_bindings; ++i) {
    ...
    if (binding->outputType == SDL_CONTROLLER_BINDTYPE_AXIS &&
      ...
      if (binding->inputType == SDL_CONTROLLER_BINDTYPE_AXIS) {
        /* FIXME: There might be multiple axes bound now that we have axis
         * ranges... */
...
int SDL_GameControllerRumble(SDL_GameController *gamecontroller,
  ...
  return SDL_JoystickRumble(SDL_GameControllerGetJoystick(gamecontroller),
...
int SDL_GameControllerRumbleTriggers(SDL_GameController *gamecontroller,
                                     ...
                                     Uint32 duration_ms) {
  return SDL_JoystickRumbleTriggers(
      SDL_GameControllerGetJoystick(gamecontroller), left_rumble, right_rumble,
      duration_ms);
}
...
SDL_GameControllerHasRumbleTriggers(SDL_GameController *gamecontroller) {
  return SDL_JoystickHasRumbleTriggers(
      SDL_GameControllerGetJoystick(gamecontroller));
}

int SDL_GameControllerSetLED(SDL_GameController *gamecontroller, Uint8 red,
                             Uint8 green, Uint8 blue) {
  return SDL_JoystickSetLED(SDL_GameControllerGetJoystick(gamecontroller), red,
                            green, blue);
}

int SDL_GameControllerSendEffect(SDL_GameController *gamecontroller,
                                 const void *data, int size) {
  return SDL_JoystickSendEffect(SDL_GameControllerGetJoystick(gamecontroller),
                                data, size);
}
...
/*
 * Event filter to transform joystick events into appropriate game controller
 * ones
 */
static int SDL_PrivateGameControllerAxis(SDL_GameController *gamecontroller,
                                         SDL_GameControllerAxis axis,
...
void SDL_GameControllerHandleDelayedGuideButton(SDL_Joystick *joystick) {
  ...
  while (controllerlist) {
    if (controllerlist->joystick == joystick) {
      SDL_PrivateGameControllerButton(
          controllerlist, SDL_CONTROLLER_BUTTON_GUIDE, SDL_RELEASED);
      break;
    }
    controllerlist = controllerlist->next;
  }
...
const char *SDL_GameControllerGetAppleSFSymbolsNameForButton(
    SDL_GameController *gamecontroller, SDL_GameControllerButton button) {
#if defined(SDL_JOYSTICK_MFI)
  const char *IOS_GameControllerGetAppleSFSymbolsNameForButton(
      SDL_GameController * gamecontroller, SDL_GameControllerButton button);
  return IOS_GameControllerGetAppleSFSymbolsNameForButton(gamecontroller,
                                                          button);
#else
  return NULL;
#endif
...
const char *SDL_GameControllerGetAppleSFSymbolsNameForAxis(
    SDL_GameController *gamecontroller, SDL_GameControllerAxis axis) {
#if defined(SDL_JOYSTICK_MFI)
  const char *IOS_GameControllerGetAppleSFSymbolsNameForAxis(
      SDL_GameController * gamecontroller, SDL_GameControllerAxis axis);
