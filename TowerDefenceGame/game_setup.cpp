...
WINDOW_KEY_LBL:
  if (eve->key.action == input::action::RELEASE) {
...
END_STREAM_LBL:
  ...
  if (states.current_key_states[value(input::key::KEY_S)]) {
    control->cam = move_parallel(control->cam, -6.f * duration);
  }

  if (states.current_key_states[value(input::key::KEY_A)]) {
    control->cam = move_perpendicular(control->cam, -6.f * duration);
  }

  if (states.current_key_states[value(input::key::KEY_D)]) {
    control->cam = move_perpendicular(control->cam, 6.f * duration);
  }

  if (states.current_key_states[value(input::key::KEY_PAGEUP)]) {
    control->cam = move_vertically(control->cam, 4.f * duration);
  }

  if (states.current_key_states[value(input::key::KEY_PAGEDOWN)]) {
    control->cam = move_vertically(control->cam, -4.f * duration);
  }
